letter-edit
    ...
    virtual(hide='{ loader }')
        .btn-group
            a.btn.btn-default(href='#settings/letter') #[i.fa.fa-chevron-left]
    ...
    script(type='text/babel').
        ...
        self.reload = () => {
            observable.trigger('letter-edit', self.item.id)
        }
            ...
            if (!self.error) {
                API.request({
                    ...
                    success(response) {
                        ...
                        self.update()
                        if (self.isNew)
                            riot.route(`/settings/letter/${self.item.id}`)
                        observable.trigger('letters-reload')
                    }
                })
            }
        }

        observable.on('letter-edit', id => {
            var params = {id}
            self.error = false
            self.isNew = false
            self.item = {}
