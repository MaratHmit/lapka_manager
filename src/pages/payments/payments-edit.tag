...
payments-edit
    ...
    virtual(hide='{ loader }')
        .btn-group
            ...
            button.btn.btn-default(if='{ isNew ? checkPermission("payments", "0100") : checkPermission("payments", "0010") }',
            onclick='{ submit }')
                i.fa.fa-floppy-o
                |  Сохранить
                            ...
                            .form-group
                                ...
                                .input-group
                                    input.form-control(value='{ item.idOrder ? "Заказ № " + item.idOrder + " от " + item.order.dateOrder + " [" + item.order.customer + "]" : "" }',
                                        readonly='{ true }')
                                    span.input-group-addon(onclick='{ selectOrder }')
                                        i.fa.fa-list
                    .row(if='{ item.idOrder }')
                        .col-md-4
                            .form-group
                            ...
                            .form-group(class='{ has-error: (error.idAuthor || error.idCompany) }')
                                ...
                                .input-group
                                    input.form-control(value='{ item.payer }',
                                        readonly='{ true }')
                                    span.input-group-addon(onclick='{ selectPayer }')
                                        i.fa.fa-list
